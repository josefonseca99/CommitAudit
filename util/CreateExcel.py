...
class CreateExcel:
    ...
    def submit_result_values(self):
        ...
        for row in range(0, len(analysts_tasks_list)):
            self.sheet.write(row + 1, 0, analysts_tasks_list[row]["analyst_email"], self.basic_format)
            self.sheet.write(row + 1, 1, analysts_tasks_list[row]["enabler_type"], self.basic_format)
            self.sheet.write(row + 1, 2, analysts_tasks_list[row]["user_story_type"], self.basic_format)
            self.sheet.write(row + 1, 3, analysts_tasks_list[row]["bug_type"], self.basic_format)
            self.sheet.write(row + 1, 4, analysts_tasks_list[row]["new_state"], self.basic_format)
            self.sheet.write(row + 1, 5, analysts_tasks_list[row]["active_state"], self.basic_format)
            self.sheet.write(row + 1, 6, analysts_tasks_list[row]["closed_state"], self.basic_format)
            self.sheet.write(row + 1, 7, analysts_tasks_list[row]["impediment_state"], self.basic_format)
            self.sheet.write(row + 1, 8, analysts_tasks_list[row]["engaged_to"], self.basic_format)
            self.sheet.write(row + 1, 9, str(analysts_tasks_list[row]["no_scored"]).strip("[]"), self.basic_format)
            self.sheet.write(row + 1, 10, analysts_tasks_list[row]["pull_data"], self.basic_format)
            self.sheet.write(row + 1, 11, analysts_tasks_list[row]["commit_data"], self.basic_format)
